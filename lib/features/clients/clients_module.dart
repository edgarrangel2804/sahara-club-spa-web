...
import 'client_identity_helpers.dart';
...
class _ClientsModuleState extends State<ClientsModule> {
  ...
  List<_Client> get _filtered {
    ...
    final qPhone = normalizeClientPhone(q);
    return _clients
        .where(
          (c) =>
              ...
              c.phone.contains(q) ||
              (qPhone.isNotEmpty &&
                  normalizeClientPhone(c.phone).contains(qPhone)),
        )
        .toList();
  }
                    ...
                    Row(
                      children: [
                        ...
                        if (widget.benefit == 'membership')
                          const _TileBadge(
                            label: 'M',
                            color: Color(0xFF6A54E0),
                            tooltip: 'Membresía activa',
                          )
                        else if (widget.benefit == 'gift_card')
                          const _TileBadge(
                            label: 'GC',
                            color: Color(0xFFE07B00),
                            tooltip: 'Gift card activa',
                          )
                        else
                          const _TileBadge(
                            label: '\$',
                            color: Color(0xFFB32D2D),
                            tooltip: 'Requiere anticipo',
                          ),
                      ],
                    ),
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('No se pudo actualizar el estatus del cliente.'),
          ),
        );
      }
    } finally {
  ...
  Future<List<Map<String, dynamic>>> _loadPackages() async {
    try {
      ...
      final pkgRows = await db
          ...
          .select('id, name, status, total_amount, paid_amount, created_at')
          ...
          children: [
            ...
            Container(
              ...
              child: Row(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  _Avatar(initials: c.initials, color: c.avatarColor, size: 64),
            ...
            ClientPackagesPanel(clientId: c.id, clientName: c.fullName),
            const SizedBox(height: 16),
...
class _ClientFormDialog extends StatefulWidget {
  const _ClientFormDialog({required this.onSaved, this.editClient});
...
class _ClientFormDialogState extends State<_ClientFormDialog> {
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      final normalizedEmail = normalizeClientEmail(_emailCtrl.text);
      final normalizedPhone = normalizeClientPhone(_phoneCtrl.text);
      final payload = {
...
class _HistoryRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final therapist =
        (booking['therapist'] as Map?)?['full_name'] as String? ?? '';
        ...
        children: [
          ...
          if (const [
            ...
            IconButton(
              ...
              onPressed: () =>
                  showDepositReceiptDialog(context, booking['id'] as String),
            ),
        ],
      ),
    );
      ...
      child: Column(
        ...
        children: [
          ...
          if (loading)
            ...
            InkWell(
              onTap: savingFrequent
                  ? null
                  : () => onToggleFrequent(!isFrequent),
              borderRadius: BorderRadius.circular(8),
              child: Container(
                  ...
                  children: [
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          Text(
                            'Cliente frecuente · exento de anticipo',
                            style: GoogleFonts.inter(
                              fontSize: 12.5,
                              fontWeight: FontWeight.w600,
                              color: Colors.black87,
            ...
            Padding(
              ...
              child: hasGc
                  ? Column(
                      ...
                      children: [
                        Text(
                          'Saldo disponible: ${_fmtMoney(gcBalance)}',
                          style: GoogleFonts.inter(
                            fontSize: 12.5,
                            color: Colors.black87,
                          ),
              ...
              child: hasMem
                  ? Column(
                      ...
                      children: [
                        ...
                        Text(
                          'Sesiones restantes: $remaining (usadas $used)',
                          style: GoogleFonts.inter(
                            fontSize: 12,
                            color: Colors.black54,
                          ),
                        ...
                        Text(
                          'Vigencia: ${_fmtDate(memStart)} → ${_fmtDate(memEnd)}',
                          style: GoogleFonts.inter(
                            fontSize: 11.5,
                            color: Colors.black54,
                          ),
            ...
            Padding(
              ...
              child: packages.isEmpty
                  ...
                  : Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [for (final p in packages) _packageRow(p)],
                    ),
            ),
...
class _PackagePaymentDialogState extends State<_PackagePaymentDialog> {
  ...
  Future<void> _load() async {
    ...
    try {
      final data = await Supabase.instance.client
          ...
          .select(
            'id, installment_number, amount, status, paid_at, payment_method',
          )
  ...
  Widget _paymentRow(Map<String, dynamic> p) {
    ...
    final paidAt = paidAtStr != null
        ? DateTime.tryParse(paidAtStr)?.toLocal()
        : null;
        ...
        children: [
          ...
          Expanded(
            child: Column(
              ...
              children: [
                Text(
                  'Abono $n · ${_fmtMoney(amount)}',
                  style: GoogleFonts.inter(fontSize: 13, color: Colors.black87),
                ),
                if (paid && paidAt != null)
                  Text(
                    'Pagado el ${fmtDate(paidAt)}',
                    style: GoogleFonts.inter(
                      fontSize: 11,
                      color: Colors.black45,
                    ),
...
class _GiftCardFormDialogState extends State<_GiftCardFormDialog> {
  ...
  Future<void> _save() async {
    ...
    if (amount <= 0) {
      setState(
        () => _error = 'El servicio seleccionado no tiene precio válido',
      );
      return;
    }
    ...
    try {
      ...
      await db.from('gift_card_transactions').insert({
        ...
        'notes':
            'Carga inicial · Servicio: $serviceName · Cobrado por $_paymentMethod',
        'created_by': me,
      });
          ...
          children: [
            ...
            Row(
              children: [
                Expanded(
                  child: Text(
                    'Vence: ${_expires.day}/${_expires.month}/${_expires.year}',
                    style: GoogleFonts.inter(fontSize: 13),
                  ),
                ),
                TextButton(
            ...
            if (_plans.isEmpty)
              ...
              Row(
                children: [
                  Expanded(
                    child: Text(
                      'Inicio: ${_start.day}/${_start.month}/${_start.year}',
                    ),
                  ),
                  TextButton(
                    onPressed: () async {
              ...
              Row(
                children: [
                  Expanded(
                    child: Text('Fin: ${_end.day}/${_end.month}/${_end.year}'),
                  ),
                  TextButton(
                    onPressed: () async {
                      final p = await showDatePicker(
...
class _PackageAssignDialogState extends State<_PackageAssignDialog> {
  ...
  Future<void> _onSelectBasePackage(String? id) async {
    ...
    final base = _basePackages.firstWhere(
      (p) => p['id'] == id,
      orElse: () => const {},
    );
    ...
    final allowed =
        ((base['installments_allowed'] as List?)
  ...
  Future<void> _save() async {
    ...
    final initialPayment =
        (double.tryParse(_initialPaymentCtrl.text.trim()) ?? 0)
            .clamp(0, total)
            .toDouble();
    setState(() {
      _saving = true;
    ...
    try {
      ...
      await db.rpc(
        ...
        params: {
          ...
          'p_name': _nameCtrl.text.trim().isEmpty
              ? 'Paquete'
              : _nameCtrl.text.trim(),
          'p_lines': snapshot,
          'p_total': total,
                          ...
                          Switch(
                            ...
                            onChanged: (v) => setState(() {
                              ...
                              if (v && _manualTotalCtrl.text.trim().isEmpty) {
                                _manualTotalCtrl.text = _linesTotal
                                    .toStringAsFixed(0);
                              }
                            }),
                          ),
...
class _BenefitsMovementsDialog extends StatefulWidget {
  ...
  @override
  State<_BenefitsMovementsDialog> createState() =>
      _BenefitsMovementsDialogState();
}
    ...
    return Dialog(
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            Text(
              'Movimientos · ${widget.client.fullName}',
              style: GoogleFonts.playfairDisplay(
                fontSize: 17,
                fontWeight: FontWeight.bold,
              ),
