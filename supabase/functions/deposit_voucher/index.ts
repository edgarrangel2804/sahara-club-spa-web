...
Deno.serve(async (req) => {
  ...
  try {
    ...
    let query = sb
      .from("bookings")
      .select("id, status, payment_status, booking_date, booking_time, deposit_amount, " +
    ...
    if (!serviceName && b.service_id) {
      const { data: svc } = await sb.from("services").select("name").eq("id", b.service_id).maybeSingle();
      serviceName = (svc as { name?: string } | null)?.name ?? "Servicio";
    }

    let clientName = "Cliente";
    if (b.client_record_id) {
      const { data: c } = await sb.from("clients").select("full_name").eq("id", b.client_record_id).maybeSingle();
