...
const SITE = 'https://saharaclubspa.com';
...
function tjNow(): Date { return new Date(new Date().toLocaleString('en-US', { timeZone: 'America/Tijuana' })); }
...
function fechasTable(): string {
  ...
  for (let i = 0; i < 45; i++) {
    ...
    out.push(`  ${DAYS[d.getDay()]} ${d.getDate()} de ${MONTHS[d.getMonth()]} → ${iso}${tag}${closed}`);
  }
  return out.join('\n');
}
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!settings || settings.ai_enabled === false || settings.ai_mode === 'disabled' || settings.ai_pause_all_conversations === true) {
      return json({ reply: 'Por ahora el asistente está en pausa. Escríbenos por WhatsApp y con gusto te ayudamos. 🤍' });
    }

    // API key de Anthropic desde el Vault (mismo método que el bot de WhatsApp).
    let apiKey = '';
    try { const { data: k } = await sb.rpc('get_anthropic_api_key'); apiKey = String(k || ''); } catch (_) { /* noop */ }
    if (!apiKey) return json({ reply: 'El asistente con IA se está conectando. Escríbenos por WhatsApp y te ayudamos. ✨' });
    ...
    const [{ data: services }, { data: faqs }, { data: hours }] = await Promise.all([
      sb.from('services').select('id,name,duration_min,price,price_on_quote,category').eq('is_active', true).order('category').order('name').limit(120),
      sb.from('faqs').select('question,answer').eq('is_active', true).eq('scope_category', 'sahara').limit(40),
      sb.from('business_hours').select('weekday,opens_at,closes_at,is_closed').eq('branch_id', BRANCH).order('weekday'),
    ]);
    ...
    const horario = (hours || []).map((h: any) =>
      `${dn[h.weekday]}: ${h.is_closed ? 'Cerrado' : `${String(h.opens_at).slice(0, 5)}–${String(h.closes_at).slice(0, 5)}`}`).join('\n');
    ...
    if (!resp.ok) { console.error('Anthropic', resp.status, await resp.text()); return json({ reply: 'Tuve un problema para responder. Escríbenos por WhatsApp y te ayudamos. 🤍' }); }
    const data = await resp.json();
    let reply = data?.content?.find((c: any) => c.type === 'text')?.text || 'Lo siento, no pude responder. Escríbenos por WhatsApp.';
    ...
    if (idx >= 0) {
      ...
      let r: any = {};
      try { const m = after.match(/\{[\s\S]*\}/); r = m ? JSON.parse(m[0]) : {}; } catch (_) { /* noop */ }
      ...
      const nombreOk = nombre.length >= 3
        && !/^(cliente(\s+whats?app|\s+web)?|whats?app|sin\s+nombre|invitad[oa]|test|prueba)$/i.test(nombre)
      ...
      if (!done && serviceId && fecha && hora && telefono && nombreOk) {
        try {
          ...
          const { data: avail } = await sb.rpc('check_availability_for_booking_from_ai', {
            p_service_id: serviceId, p_requested_date: fecha, p_requested_time: timeNorm,
            p_duration_min: null, p_branch_id: BRANCH, p_staff_id: null,
          });
          if ((avail as any)?.available === true) {
            const { data: created } = await sb.rpc('create_pending_booking_from_ai', {
              p_phone: telefono, p_client_name: nombre, p_email: email || null, p_service_id: serviceId,
              p_booking_date: fecha, p_booking_time: timeNorm, p_duration_min: null,
              p_notes: 'Solicitud creada por concierge web.',
              p_ai_conversation_id: null, p_ai_confidence_score: 0.9, p_therapist_id: null,
            });
            ...
            if (bookingId) {
              ...
              const { data: req } = await sb.rpc('check_booking_payment_requirement', {
                p_phone: telefono, p_service_id: serviceId, p_requested_date: fecha,
                p_requested_time: timeNorm, p_customer_name: nombre,
              });
              ...
              if (requiresDeposit && depositEnabled) {
                ...
                await sb.from('bookings').update({
                  status: 'pending_payment', payment_requirement: 'deposit_required',
                  deposit_required_cents: cents, deposit_amount: cents / 100, payment_status: 'pending',
                }).eq('id', bookingId);
                ...
                let payUrl = '';
                try {
                  const { data: co } = await sb.functions.invoke('create_booking_deposit_checkout', {
                    body: { booking_id: bookingId, amount: cents / 100 },
                  });
                  payUrl = String((co as any)?.checkout_url ?? '');
                } catch (e) { console.error('checkout web', e); }
                if (payUrl) {
                  reply += `\n\nPara apartar tu cita realiza el anticipo de $${cents / 100} MXN aquí:\n${payUrl}\nAl completar el pago tu cita queda confirmada y te enviamos tu comprobante. 🌿`;
                } else {
                  reply += `\n\nPara apartar tu cita se requiere un anticipo de $${cents / 100} MXN. Recepción te enviará el link de pago en breve. 🌿`;
                }
              } else {
        ...
        } catch (e) { console.error('reserva web', e); }
      }
      if (!done) {
